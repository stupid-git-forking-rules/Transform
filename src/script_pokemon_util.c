...
void CreateScriptedWildMon(u16 species, u8 level, u16 item, u8 isShiny, u16 move1, u16 move2, u16 move3, u16 move4, u8 ability)
{
    u8 heldItem[2];
    ...
    if (ability)
    {
        monAbility = ability - 1; 
        SetMonData(&gEnemyParty[0], MON_DATA_ABILITY_NUM, &monAbility);
    }
    if (item)
    {
        heldItem[0] = item;
